...
import { COLUMN_ATTENDANCE_EMAIL, COLUMN_ATTENDANCE_NAME, COLUMN_ATTENDANCE_PHONE, COLUMN_ATTENDANCE_TIMESTAMP, COLUMN_PARTICIPANT_EMAIL, COLUMN_PARTICIPANT_ID, COLUMN_PARTICIPANT_NAME, COLUMN_PARTICIPANT_PHONE, COLUMN_PARTICIPANT_REMEDIAL } from "../constants";
...
export default async function attendanceGeneratorConversation(conversation: Conversation<Context>, ctx: Context) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    do {
      resCtx = await conversation.wait() as MyContext;
      if (resCtx.update && resCtx.update.callback_query) {
        switch (resCtx.update.callback_query.data) {
          case "cancel":
            await resCtx.reply("Operasi dibatalkan.");
            return;
          default:
  ...
  function searchTimestamp(participant: Array<string|number|boolean|null>, no: number): [string|null, string|null] {
    let current = data[no] as Array<Array<string|number|boolean>>;
  ...
  for (let i = 1; i < data.participants.length; i++) {
    ...
    insert.concat(searchTimestamp(insert, 1));
    if (data[2]) insert.concat(searchTimestamp(insert, 2));
    if (data[3]) insert.concat(searchTimestamp(insert, 3));
  }
  ...
  resWb.Sheets[resSheetNames[0]] = XLSX.utils.aoa_to_sheet(data.final);
  if (warnings.length > 0) {
    if (resFormat != "csv"){
      resWb.Sheets[resSheetNames[1]] = XLSX.utils.aoa_to_sheet(([["No", "Catatan"]] as Array<string|number>[]).concat(warnings.map((value, index) => [index + 1, value])));
    } else {
      await resCtx2.reply(["⚠️ Warning!"].concat(warnings).join("\n"));
    }
  }

  const u8 = XLSX.write(resWb, { bookType: resFormat as XLSX.BookType });
